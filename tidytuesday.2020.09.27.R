...
death_rate <- exp[, .(members = sum(members), 
                  ...
                  .(oxygen_used, expedition_id)]
death_rate <- death_rate[, death_rate := ifelse(deaths/members == 0 , "0%",
                                                ifelse(deaths/members < .10, "0% < 10%",
                                                       "10% <"))]
dr <- death_rate[!is.na(death_rate), .N, .(oxygen_used, death_rate)] %>% 
  ...
  scale_y_continuous(limits = c(0, 8300)) +
  theme_bw() + xlab(NULL) + ylab(NULL) + labs(title = "Freq. of Party Deaths %") +
